...
// adding cmyk > added in UI xml too
uniform float Cyan;
...
float BlackA = min(Cyan, Magenta);
float Black = min(BlackA, Yellow); 
float newCyan = Cyan - Black;
float newMagenta = Magenta - Black;
float newYellow = Yellow - Black;

float newRed = Red + (1 - newCyan) * (1 - Black);
float newGreen = Green + (1 - newMagenta) * (1 - Black);
float newBlue = Blue + (1 - newYellow) * (1 - Black);
